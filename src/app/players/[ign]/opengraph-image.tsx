...
export default async function Image({ params }: { params: { ign: string } }) {
  ...
  const p = data.players.find((x) => x.ign.toLowerCase() === decodeURIComponent(params.ign).toLowerCase());
  ...
  return new ImageResponse(
    (
      <div style={{ fontSize: 52, color: "#e6eefc", width: "100%", height: "100%", display: "flex", background: "linear-gradient(120deg, #0a0f1c, #0b1630)", padding: "40px", alignItems: "center", justifyContent: "space-between" }}>
        ...
        {logo && <img src={logo} width={150} height={150} alt="Team Logo" style={{ borderRadius: 16, background: "rgba(255,255,255,.06)", padding: 12 }} />}
      </div>
    ),
    size
  );
